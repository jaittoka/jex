...
module.exports = class Compiler
  ...
  compileNode: (node) ->
    ...
    return compilers['var'] node.value, @ if @isIdentifier node
    if @isArray node
      @compileApply node
    else
      JSON.stringify node
  ...
  compile: (node) -> 
    ...
    { code: res, refs: (name for name, _ of @globals) }

  compileToFunc: (node) ->
    # Compile the node
    { code, refs } = @compile node
